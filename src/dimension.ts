...
import { GENERATOR_UPGRADES } from "./generator-upgrades";
...
export function dimensionCost(x: number): Decimal {
  ...
  if (
    player.dimensions[x][1].gt(20) &&
    x == 0 &&
    GENERATOR_UPGRADES.upgradeAmount(0).lt(1)
  ) {
    return new Decimal(1 / 0);
  }
  if (
    player.dimensions[x][1].gt(57) &&
    x == 1 &&
    GENERATOR_UPGRADES.upgradeAmount(0).lt(1)
  ) {
    return new Decimal(1 / 0);
  ...
  if (player.dimensions[x][1].gte(310) && x == 1) {
    return player.dimensions[x][1].sub(310).pow_base(2).mul(1e24);
  }
  ...
  if (
    player.dimensions[x][1].gte(3) &&
    x == 2 &&
    GENERATOR_UPGRADES.upgradeAmount(0).lt(1)
  )
    return new Decimal(1 / 0);
  if (
    player.dimensions[x][1].gte(1) &&
    x == 3 &&
    GENERATOR_UPGRADES.upgradeAmount(0).lt(1)
  )
    return new Decimal(1 / 0);
...
export function dimensionMult(x: number): Decimal {
  let q = player.dimensions[x][1].pow_base(1.2);

  if (GENERATOR_UPGRADES.upgradeAmount(0).lt(1)) q = q.clampMax(75);
  else if (q.gte(75)) {
    q = q.div(75).pow(0.5).mul(75);
  }
...
export function dimLoop(seconds: number) {
  ...
  for (let i = 1; i < 4; i++) {
    player.dimensions[i - 1][0] = player.dimensions[i - 1][0]
      ...
      .clampMax(GENERATOR_UPGRADES.upgradeAmount(0).lt(1) ? 1e9 : 1 / 0);
  }
}
